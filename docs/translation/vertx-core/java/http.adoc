...
你需要确保在类路径classpath上存在有 Brotli4j 以解压 Brotli：
...
[[_http_compression_algorithms]]
=== HTTP 压缩算法

Vert.x 支持开箱即用的 deflate 和 gzip。

也可以使用 Brotli 和 zstandard。
...
NOTE: 使用 {@code io.netty.handler.codec.compression.StandardCompressionOptions} 静态方法创建 {@code io.netty.handler.codec.compression.CompressionOptions}

Brotli 和 zstandard 库需要添加到类路径classpath中。
