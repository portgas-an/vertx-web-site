...
你可以使用 Json Schema 去校验每一个json结构的数据。该组件提供以下特性：
...
顾名思义，`link:../../apidocs/io/vertx/json/schema/Validator.html[Validator]` 使用一个起始 schema 以校验对象。
输出格式则取决于配置。
...
由于schemas可以包含引用，因此校验器和存储仓库需要获取您的应用程序的 `baseUri 基础通用资源识别号`。
...
您可以将 `JsonSchema` 实例用于不同的 `Validator 校验器`，且可直接使用 `JsonParser` 解析不同的 `JsonSchema`。
