...
`link:../../apidocs/io/vertx/mysqlclient/MySQLPool.html[MySQLPool]` 允许您创建连接池或池化的客户端
...
为了在线上获得最佳性能，我们以管线/流水线模式执行批处理，这意味着执行请求在先前请求的响应返回之前发送，如果您的服务器或代理不支持此功能或您可能对此不感兴趣，您可以自行使用单个执行 API 编码。
