...
<<reserved_set,保留>>集以及<<unreserved_set,非保留>>集以外的文字字符则采用 https://en.wikipedia.org/wiki/Percent-encoding[百分比编码]：
...
简单的字符串扩展则将 <<unreserved_set,非保留>> 集合之外的任何字符编码为其百分比编码等效项：
